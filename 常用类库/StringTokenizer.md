...
- 在调试Spring源码的时候，发现`org.springframework.beans.factory.xml.BeanDefinitionParserDelegate#parseBeanDefinitionElement(org.w3c.dom.Element, org.springframework.beans.factory.config.BeanDefinition)`方法当中分割name属性使用到了`StringUtils.tokenizeToStringArray`方法，进入底层，发现使用的是`StringTokenizer`的构造器方法
- 所以再开一篇文档记录一下`StringTokenizer`的用法
- 方法是属于`java.util;`包下面的

# StringTokenizer的常见方法
